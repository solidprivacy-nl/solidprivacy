...
1. Read and apply the canonical operating method beginning at `market-predictions/control-plane/control/SYSTEM_INDEX.md`.
2. Read this bootstrap.
3. Read `CURRENT_STATE.md`.
4. Read `ROADMAP.md`.
5. Read `WORKPACKAGES.md` and the relevant detailed workpackage(s) in `docs/workpackages/`.
6. Read `CHANGELOG.md` and `DECISION_LOG.md` for durable changes affecting the assignment.
7. Read `control/WORK_CLAIMS.json` and live-reconcile relevant claims/branches/PRs under the canonical lifecycle standard.
8. Read relevant records under `handover/`.
9. Read `docs/architecture.md`, `docs/DATA_ARCHITECTURE.md` and other architecture contracts relevant to scope.
10. Inspect the actual source, tests, GitHub Actions and exact-head evidence; narrative completion claims are not sufficient.
...
- `CURRENT_STATE.md` is the project-local human-readable current-state view and must be reconciled against live GitHub before use.
...
- No chat message, assistant memory or external planning document may silently override repository state.
...
Assurance may not silently repair the candidate it is reviewing. Any repair creates a new candidate requiring a fresh assurance cycle.
...
`.github/workflows/project-governance-gate.yml` must validate the static project-governance manifest, mandatory source-of-truth files and the basic work-claim contract on consequential branches/PRs.

Machine governance does not replace live branch reconciliation or independent assurance; it prevents obvious structural drift.

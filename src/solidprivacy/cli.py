...
from solidprivacy.runtime.review import build_review_request, finalize_dpia_review, verify_audit_record
...
def _fixture_review_dpia(request_path: str, policy_path: str, result_path: str, decisions_path: str) -> int:
    request=_load(request_path); policy=_load(policy_path); result=_load(result_path); decisions=_load(decisions_path)
    ...
    _dump({"review_request":review_request,"review_package":package}); return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p=sub.add_parser("fixture-review-dpia"); p.add_argument("request"); p.add_argument("policy"); p.add_argument("result"); p.add_argument("decisions")
    return parser
...
def main() -> int:
    ...
    if args.command=="fixture-review-dpia": return _fixture_review_dpia(args.request,args.policy,args.result,args.decisions)

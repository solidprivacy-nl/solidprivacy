...
def dpia_integrity_errors(dpia: dict[str, Any]) -> list[str]:
    ...
    global_subject_id_list: list[str] = []
    global_personal_data_id_list: list[str] = []
    global_purpose_id_list: list[str] = []
    global_party_id_list: list[str] = []
    ...
    for index, activity in enumerate(activities):
        ...
        for pd_index, item in enumerate(personal_data):
            _check_refs(
                errors,
                item.get("data_subject_ids"),
                local_subjects,
                f"{prefix}.personal_data[{pd_index}].data_subject_ids",
            )

        for retention_index, item in enumerate(activity.get("retention", [])):
            _check_refs(
                errors,
                item.get("purpose_ids"),
                local_purposes,
                f"{prefix}.retention[{retention_index}].purpose_ids",
            )
            _check_refs(
                errors,
                item.get("data_subject_ids"),
                local_subjects,
                f"{prefix}.retention[{retention_index}].data_subject_ids",
            )
            _check_refs(
                errors,
                item.get("personal_data_ids"),
                local_personal_data,
                f"{prefix}.retention[{retention_index}].personal_data_ids",
            )

        global_subject_id_list.extend(subject_ids)
        global_personal_data_id_list.extend(personal_data_ids)
        global_purpose_id_list.extend(purpose_ids)
        global_party_id_list.extend(party_ids)

    for label, values in (
        ("data_subjects", global_subject_id_list),
        ("personal_data", global_personal_data_id_list),
        ("purposes", global_purpose_id_list),
        ("parties", global_party_id_list),
    ):
        for duplicate in _duplicates(values):
            errors.append(
                f"processing_activities.{label}: globally duplicate id {duplicate!r}"
            )

    global_subject_ids = set(global_subject_id_list)
    global_personal_data_ids = set(global_personal_data_id_list)
    ...
    for index, risk in enumerate(dpia.get("risks", [])):
        _check_refs(
            errors,
            risk.get("processing_activity_ids"),
            activity_id_set,
            f"risks[{index}].processing_activity_ids",
        )
        _check_refs(
            errors,
            risk.get("affected_data_subjects"),
            global_subject_ids,
            f"risks[{index}].affected_data_subjects",
        )

    evidence_ids = {
        item.get("id")
        for item in dpia.get("evidence", [])
        if item.get("id")
    }
    ...
    for index, measure in enumerate(dpia.get("measures", [])):
        _check_refs(
            errors,
            measure.get("risk_ids"),
            risk_id_set,
            f"measures[{index}].risk_ids",
        )
        _check_refs(
            errors,
            measure.get("evidence_refs"),
            evidence_ids,
            f"measures[{index}].evidence_refs",
        )
    ...
    for index, item in enumerate(dpia.get("purpose_compatibility", [])):
        ...
        if activity_id and activity_id not in activity_id_set:
            errors.append(
                f"purpose_compatibility[{index}].processing_activity_id: "
                f"unknown reference {activity_id!r}"
            )

    for index, item in enumerate(dpia.get("special_data_assessment", [])):
        _check_refs(
            errors,
            item.get("processing_activity_ids"),
            activity_id_set,
            f"special_data_assessment[{index}].processing_activity_ids",
        )
        _check_refs(
            errors,
            item.get("personal_data_ids"),
            global_personal_data_ids,
            f"special_data_assessment[{index}].personal_data_ids",
        )
        _check_refs(
            errors,
            item.get("data_subject_ids"),
            global_subject_ids,
            f"special_data_assessment[{index}].data_subject_ids",
        )
    ...
    if residual.get("level") == "high":
        if residual.get("prior_consultation_required") is False:
            errors.append(
                "residual_risk_conclusion.prior_consultation_required: "
                "must not be false while residual risk is high"
            )
        if not review.get("required"):
            errors.append(
                "human_review.required: must be true while residual risk is high"
            )
        if review.get("status") in {"approved", "approved_with_changes"}:
            errors.append(
                "human_review.status: high residual risk cannot be treated as approved "
                "without resolving the high-risk gate"
            )
